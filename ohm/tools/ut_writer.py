...
from ohm.provider.openai_api import OpenAILLM as GPTAPI
...
class UTGenerator:
    ...
    async def ask_gpt_and_save(self, question: str, tag: str, fname: str):
        ...
        await async_write(Path(self.questions_path) / tag / f"{fname}.txt", question)
        data = result.get("code", "") if result else ""
        await async_write(Path(self.ut_py_path) / tag / f"{fname}.py", data)
