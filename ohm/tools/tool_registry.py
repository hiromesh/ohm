...
from ohm.const import TOOL_SCHEMA_PATH
from ohm.logs import logger
from ohm.tools.tool_convert import (
    convert_code_to_tool_schema,
    convert_code_to_tool_schema_ast,
)
from ohm.tools.tool_data_type import Tool, ToolSchema
...
def register_tool(tags: list[str] = None, schema_path: str = "", **kwargs):
    ...
    def decorator(cls):
        ...
        if "ohm" in file_path:
            # split to handle ../ohm/ohm/tools/... where only metapgt/tools/... is needed
            file_path = "ohm" + file_path.split("ohm")[-1]
        source_code = ""
