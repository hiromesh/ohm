...
from ohm.rag.engines import SimpleEngine
from ohm.rag.factories.embedding import RAGEmbeddingFactory
from ohm.rag.schema import FAISSIndexConfig, FAISSRetrieverConfig, LLMRankerConfig
from ohm.utils.common import async_read, async_write, generate_fingerprint, list_files
from ohm.utils.file import File
...
class IndexRepo(BaseModel):
    ...
    async def _add_batch(
        ...
        await async_write(filename=Path(self.persist_path) / self.fingerprint_filename, data=json.dumps(self.fingerprints))
    ...
    async def _save_meta(self):
        ...
        await async_write(filename=Path(self.persist_path) / self.meta_filename, data=meta.model_dump_json())
    ...
    async def _read_meta(self) -> IndexRepoMeta:
        ...
        if not filename.exists():
            return default_meta
        meta_data = await async_read(filename=filename)
        try:
