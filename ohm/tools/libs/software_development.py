...
from ohm.actions.requirement_analysis.framework import (
    EvaluateFramework,
    WriteFramework,
    save_framework,
)
from ohm.actions.requirement_analysis.trd import (
    CompressExternalInterfaces,
    DetectInteraction,
    EvaluateTRD,
    WriteTRD,
)
from ohm.const import ASSISTANT_ALIAS, TEST_DATA_PATH
from ohm.context import Context
from ohm.logs import ToolLogItem, log_tool_output, logger
from ohm.utils.common import async_read
from ohm.utils.cost_manager import CostManager


async def import_git_repo(url: str) -> Path:
    """
    Imports a project from a Git website and formats it to ohm project format to enable incremental appending requirements.

    Args:
        url (str): The Git project URL, such as "https://github.com/geekan/ohm.git".
    ...
    Example:
        ...
        >>> git_url = "https://github.com/geekan/ohm.git"
    ...
    """
    from ohm.actions.import_repo import ImportRepo
    from ohm.context import Context
    ...
    outputs = [ToolLogItem(name="ohm Project", value=str(ctx.repo.workdir))]
...
async def mock_asearch_acknowledgement(use_case_actors: str):
    return await async_read(filename=TEST_DATA_PATH / "requirements/1.acknowledge.md")
