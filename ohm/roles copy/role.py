...
from ohm.actions import Action, ActionOutput
from ohm.actions.action_node import ActionNode
from ohm.actions.add_requirement import UserRequirement
from ohm.base import BaseEnvironment, BaseRole
from ohm.const import MESSAGE_ROUTE_TO_SELF
from ohm.context_mixin import ContextMixin
from ohm.logs import logger
from ohm.memory import Memory
from ohm.provider import HumanProvider
...
from ohm.utils.common import any_to_name, any_to_str, role_raise_decorator
from ohm.utils.repair_llm_raw_output import extract_state_value_from_output
...
class RoleContext(BaseModel):
    ...
    env: BaseEnvironment = Field(default=None,
                                 exclude=True)  # # avoid circular import
...
class Role(BaseRole, SerializationMixin, ContextMixin, BaseModel):
    ...
    latest_observed_msg: Optional[
        Message] = None  # record the latest observed message when interrupted
    ...
    def check_addresses(self):
        if not self.addresses:
            self.addresses = {any_to_str(self), self.name
                              } if self.name else {any_to_str(self)}
        return self
    ...
    def _set_react_mode(self,
        ...
        """
        assert react_mode in RoleReactMode.values(
        ...
        if react_mode == RoleReactMode.REACT:
            ...
            self.planner = Planner(goal=self.goal,
    ...
    def set_env(self, env: BaseEnvironment):
        ...
        if env:
            ...
            self.llm.cost_manager = self.context.cost_manager
            self.set_actions(
                self.actions)  # reset actions to update llm and prefix
    ...
    def _get_prefix(self):
        ...
        if self.rc.env and self.rc.env.desc:
            ...
            other_role_names = ", ".join(
                [r for r in all_roles if r != self.name])
    ...
    async def _think(self) -> bool:
        ...
        if self.recovered and self.rc.state >= 0:
            self._set_state(
                self.rc.state)  # action to run from recovered state
        ...
        if (not next_state.isdigit()
                and next_state != "-1") or int(next_state) not in range(
                    -1, len(self.states)):
            logger.warning(
    ...
    async def _observe(self) -> int:
        ...
        self.latest_observed_msg = self.rc.news[
            -1] if self.rc.news else None  # record the latest observed msg
    ...
    def publish_message(self, msg):
        ...
        if all(to in {any_to_str(self), self.name}
               for to in msg.send_to):  # Message to myself
    ...
    async def _react(self) -> Message:
        ...
        Use llm to select actions in _think dynamically
        """
        logger.info(f'{self._setting}: ?????start react mode!!!\n\n')
        actions_taken = 0
        ...
        while actions_taken < self.rc.max_react_loop:
            ...
            rsp = await self._act()
            actions_taken += 1
        logger.info(f"{self._setting}\n\n{self.rc.state=}\n\nwill do {self.rc.todo}\n\nactions taken: {actions_taken}\n\nresponse: {rsp}")
        return rsp  # return output from the last action

    async def _plan_and_act(self) -> Message:
        """first plan, then execute an action sequence, i.e. _think (of a plan) -> _act -> _act -> ... Use llm to come up with the plan dynamically."""
        logger.info(f'{self._setting}: start plan and act mode!!!\n\n')

        if not self.planner.plan.goal:
            # create initial plan and update it until confirmation
            goal = self.rc.memory.get(
            )[-1].content  # retreive latest user requirement
        ...
        rsp = self.planner.get_useful_memories()[
            0]  # return the completed plan as a response
        rsp.role = "assistant"
    ...
    async def react(self) -> Message:
        ...
        if self.rc.react_mode == RoleReactMode.REACT or self.rc.react_mode == RoleReactMode.BY_ORDER:
            logger.info(f"{self._setting}: 1start react mode!!!\n\n")
            rsp = await self._react()
        elif self.rc.react_mode == RoleReactMode.PLAN_AND_ACT:
            logger.info(f"{self._setting}: 2start plan and act mode!!!\n\n")
    ...
    async def run(self, with_message=None) -> Message | None:
        ...
        if not await self._observe():
            ...
            logger.info(f"{self.name} {self._setting}: no news. waiting.")
            return
        logger.info(f"{self.name} {self._setting}: start react\n\n")
    ...
    async def think(self) -> Action:
        ...
        """
        await self._observe(
        )  # For compatibility with the old version of the Agent.
        await self._think()
        return self.rc.todo
