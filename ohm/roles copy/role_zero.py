...
from ohm.actions import Action, UserRequirement
from ohm.actions.di.run_command import RunCommand
from ohm.actions.search_enhanced_qa import SearchEnhancedQA
from ohm.const import IMAGES
from ohm.exp_pool import exp_cache
from ohm.exp_pool.context_builders import RoleZeroContextBuilder
from ohm.exp_pool.serializers import RoleZeroSerializer
from ohm.logs import logger
from ohm.memory.role_zero_memory import RoleZeroLongTermMemory
from ohm.prompts.di.role_zero import (
    ASK_HUMAN_COMMAND,
...
from ohm.schema import AIMessage, Message, UserMessage
from ohm.strategy.experience_retriever import DummyExpRetriever, ExpRetriever
from ohm.strategy.planner import Planner
from ohm.tools.libs.browser import Browser
from ohm.tools.libs.editor import Editor
from ohm.tools.tool_recommend import BM25ToolRecommender, ToolRecommender
from ohm.tools.tool_registry import register_tool
from ohm.utils.common import CodeParser, any_to_str, extract_and_encode_images
from ohm.utils.repair_llm_raw_output import (
    RepairType,
    repair_escape_error,
    repair_llm_raw_output,
)
from ohm.utils.report import ThoughtReporter
...
class RoleZero(Role):
    ...
    goal: str = ""
    system_msg: Optional[list[
        str]] = None  # Use None to conform to the default value at llm.aask
    ...
    tool_execution_map: Annotated[dict[str, Callable],
                                  Field(exclude=True)] = {}
    special_tool_commands: list[str] = [
        "Plan.finish_current_task", "end", "Terminal.run_command",
        "RoleZero.ask_human"
    ]
    ...
    # Experience
    experience_retriever: Annotated[ExpRetriever,
                                    Field(exclude=True)] = DummyExpRetriever()
    ...
    def set_plan_and_tool(self) -> "RoleZero":
        ...
        self._set_react_mode(react_mode=self.react_mode,
    ...
    def set_tool_execution(self) -> "RoleZero":
        ...
        if self.config.enable_search:
            self.tool_execution_map["SearchEnhancedQA.run"] = SearchEnhancedQA(
            ).run
        ...
        if self.config.role_zero.enable_longterm_memory:
            ...
            self.rc.memory = RoleZeroLongTermMemory(
                **self.rc.memory.model_dump(),
                persist_path=self.config.role_zero.
                longterm_memory_persist_path,
    ...
    async def llm_cached_aask(self, *, req: list[dict], system_msgs: list[str],
                              **kwargs) -> str:
    ...
    async def parse_browser_actions(self,
                                    memory: list[Message]) -> list[Message]:
        if not self.browser.is_empty_page:
            ...
            for index, msg in zip(range(len(memory), 0, -1), memory[::-1]):
                if pattern.search(msg.content):
                    memory.insert(
                        index,
                        UserMessage(cause_by="browser",
                                    content=await self.browser.view()))
                    break
        return memory

    async def parse_editor_result(self,
                                  memory: list[Message],
                                  keep_latest_count=5) -> list[Message]:
        ...
        for msg in reversed(memory):
            ...
            if matches:
                ...
                if i > keep_latest_count:
                    new_content = msg.content[:msg.content.
                                              find("Command Editor")]
                    new_content += "\n".join([
                        f"Command Editor.{match} executed."
    ...
    async def _act(self) -> Message:
        ...
        commands, ok, self.command_rsp = await self._parse_commands(
            self.command_rsp)
        self.rc.memory.add(AIMessage(content=self.command_rsp))
        if not ok:
            error_msg = commands
            self.rc.memory.add(
                UserMessage(content=error_msg, cause_by=RunCommand))
            return error_msg
        ...
        return AIMessage(
            content=
            f"I have finished the task, please mark my task as finished. Outputs: {outputs}",
            sent_from=self.name,
            cause_by=RunCommand,
        )
    ...
    async def _react(self) -> Message:
        ...
        logger.info(f"{self._setting}: 1111start react mode!!!\n\n")
        self._set_state(0)
        ...
        actions_taken = 0
        rsp = AIMessage(content="No actions taken yet",
                        cause_by=Action)  # will be overwritten after Role _act
    ...
    async def _quick_think(self) -> Tuple[Message, str]:
        ...
        if answer:
            self.rc.memory.add(
                AIMessage(content=answer, cause_by=QUICK_THINK_TAG))
        ...
        if command_rsp in past_rsp and '"command_name": "end"' not in command_rsp:
            ...
            if past_rsp.count(command_rsp) >= 3:
                if '"command_name": "Plan.finish_current_task",' in command_rsp:
                    ...
                    logger.warning(
                        f"Duplicate response detected: {command_rsp}")
                    return END_COMMAND
                problem = await self.llm.aask(req + [
                    UserMessage(content=SUMMARY_PROBLEM_WHEN_DUPLICATE.format(
                        language=self.respond_language))
                ])
                ASK_HUMAN_COMMAND[0]["args"][
                    "question"] = ASK_HUMAN_GUIDANCE_FORMAT.format(
                        problem=problem).strip()
                ask_human_command = "```json\n" + json.dumps(
                    ASK_HUMAN_COMMAND, indent=4, ensure_ascii=False) + "\n```"
    ...
    async def _parse_commands(self, command_rsp) -> Tuple[List[Dict], bool]:
        ...
        """
        try:
            commands = CodeParser.parse_code(block=None,
                                             lang="json",
                                             text=command_rsp)
            if commands.endswith("]") and not commands.startswith("["):
                commands = "[" + commands
            commands = json.loads(
                repair_llm_raw_output(output=commands,
                                      req_keys=[None],
            ...
            commands = await self.llm.aask(msg=JSON_REPAIR_PROMPT.format(
                json_data=command_rsp, json_decode_error=str(e)))
            try:
                commands = json.loads(
                    CodeParser.parse_code(block=None,
                                          lang="json",
                                          text=commands))
                ...
                commands = json.loads(
                    repair_llm_raw_output(output=new_command,
                                          req_keys=[None],
                                          repair_type=RepairType.JSON))
        ...
        if isinstance(commands, dict):
            commands = commands["commands"] if "commands" in commands else [
                commands
            ]
        ...
        if command_flag.count(False) > 1:
            ...
            command_rsp = "```json\n" + json.dumps(
                commands, indent=4, ensure_ascii=False) + "\n```"
            logger.info(
    ...
    def _get_plan_status(self) -> Tuple[str, str]:
        ...
        current_task = (self.planner.plan.current_task.model_dump(
            exclude=["code", "result", "is_success"])
                        if self.planner.plan.current_task else "")
        # format plan status
        # Example:
    ...
    async def ask_human(self, question: str) -> str:
        ...
        from ohm.environment.env import MGXEnv  # avoid circular import
    ...
    async def _end(self, **kwarg):
        ...
        if not any([
                "reply_to_human" in memory.content
                for memory in self.get_memories(k=5)
        ]):
            ...
            async with ThoughtReporter(enable_llm_stream=True) as reporter:
                ...
                reply_content = await self.llm.aask(
                    self.llm.format_msg(memory +
                                        [UserMessage(reply_to_human_prompt)]))
            await self.reply_to_human(content=reply_content)
            self.rc.memory.add(
                AIMessage(content=reply_content, cause_by=RunCommand))
        outputs = ""
