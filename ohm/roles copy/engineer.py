...
from ohm.actions import WriteCode, WriteCodeReview, WriteTasks
from ohm.actions.fix_bug import FixBug
from ohm.actions.prepare_documents import PrepareDocuments
from ohm.actions.project_management_an import REFINED_TASK_LIST, TASK_LIST
from ohm.actions.summarize_code import SummarizeCode
from ohm.actions.write_code_plan_and_change_an import WriteCodePlanAndChange
...
from ohm.utils.git_repository import ChangeType
from ohm.utils.project_repo import ProjectRepo
...
class Engineer(Role):
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self._watch([
            WriteTasks, SummarizeCode, WriteCode, WriteCodeReview, FixBug,
    ...
    async def _act_sp_with_cr(self, review=False) -> Set[str]:
        ...
        for todo in self.code_todos:
            ...
            if self.config.inc:
                dependencies.add(
                    coding_context.code_plan_and_change_doc.root_relative_path)
    ...
    async def _act_summarize(self):
        ...
        for todo in self.summarize_todos:
            ...
            summary_filename = Path(
                todo.i_context.design_filename).with_suffix(".md").name
            dependencies = {
                todo.i_context.design_filename, todo.i_context.task_filename
            }
            ...
            if not is_pass:
                ...
                await self.repo.docs.code_summary.delete(
                    filename=Path(todo.i_context.design_filename).name)
        self.summarize_todos = []
        logger.info(
            f"--max-auto-summarize-code={self.config.max_auto_summarize_code}")
        if not tasks or self.config.max_auto_summarize_code == 0:
            self.n_summarize = 0
            kvs = self.input_args.model_dump()
            kvs["changed_src_filenames"] = [
                str(self.repo.srcs.workdir / i)
                for i in list(self.repo.srcs.changed_files.keys())
            ]
            if self.repo.docs.code_plan_and_change.changed_files:
                kvs["changed_code_plan_and_change_filenames"] = [
                    ...
                    for i in list(self.repo.docs.code_plan_and_change.
                                  changed_files.keys())
                ]
            if self.repo.docs.code_summary.changed_files:
                kvs["changed_code_summary_filenames"] = [
                    str(self.repo.docs.code_summary.workdir / i) for i in list(
                        self.repo.docs.code_summary.changed_files.keys())
                ]
            return AIMessage(
                content=
                f"Coding is complete. The source code is at {self.repo.workdir.name}/{self.repo.srcs.root_path}, containing: "
                + "\n".join(
                    list(self.repo.resources.code_summary.changed_files.keys())
                    + list(self.repo.srcs.changed_files.keys()) +
                    list(self.repo.resources.code_plan_and_change.
                         changed_files.keys())),
                instruct_content=AIMessage.create_instruct_value(
                    kvs=kvs, class_name="SummarizeCodeOutput"),
    ...
    async def _act_code_plan_and_change(self):
        ...
        dependencies = {
            REQUIREMENT_FILENAME,
            str(
                Path(self.rc.todo.i_context.prd_filename).relative_to(
                    self.repo.workdir)),
            str(
                Path(self.rc.todo.i_context.design_filename).relative_to(
                    self.repo.workdir)),
            str(
                Path(self.rc.todo.i_context.task_filename).relative_to(
                    self.repo.workdir)),
        }
        code_plan_and_change_filepath = Path(
            self.rc.todo.i_context.design_filename)
        await self.repo.docs.code_plan_and_change.save(
            filename=code_plan_and_change_filepath.name,
    ...
    async def _is_pass(self, summary) -> (str, str):
        rsp = await self.llm.aask(msg=IS_PASS_PROMPT.format(context=summary),
                                  stream=False)
        logger.info(rsp)
        if "YES" in rsp:
            return True, rsp
    ...
    async def _think(self) -> bool:
        ...
        write_plan_and_change_filters = any_to_str_set(
            [PrepareDocuments, WriteTasks, FixBug])
        write_code_filters = any_to_str_set(
            [WriteTasks, WriteCodePlanAndChange, SummarizeCode])
        summarize_code_filters = any_to_str_set([WriteCode, WriteCodeReview])
        if self.config.inc and msg.cause_by in write_plan_and_change_filters:
            logger.debug(
                f"TODO WriteCodePlanAndChange:{msg.model_dump_json()}")
        ...
        if msg.cause_by in write_code_filters:
            ...
            return bool(self.rc.todo)
        if msg.cause_by in summarize_code_filters and msg.sent_from == any_to_str(
                self):
    ...
    async def _new_coding_context(self, filename,
                                  dependency) -> Optional[CodingContext]:
        old_code_doc = await self.repo.srcs.get(filename)
        if not old_code_doc:
            old_code_doc = Document(root_path=str(self.repo.src_relative_path),
                                    filename=filename,
                                    content="")
        dependencies = {
            Path(i)
            for i in await dependency.get(old_code_doc.root_relative_path)
        }
        task_doc = None
        design_doc = None
        code_plan_and_change_doc = await self._get_any_code_plan_and_change(
        ) if await self._is_fixbug() else None
    ...
    async def _new_coding_doc(self, filename,
                              dependency) -> Optional[Document]:
        context = await self._new_coding_context(filename, dependency)
        if not context:
            return None  # `__init__.py` created by `init_python_folder`
        coding_doc = Document(root_path=str(self.repo.src_relative_path),
                              filename=filename,
    ...
    async def _new_code_actions(self):
        ...
        for filename in changed_task_filenames:
            ...
            if hasattr(self.input_args, "changed_system_design_filenames"):
                ...
                changed_system_design_filenames = [
                    str(self.repo.docs.system_design.workdir / i) for i in
            ...
            for i in changed_code_plan_and_change_filenames:
                if task_filename.name == Path(i).name:
                    ...
                    break
            design_doc = await Document.load(filename=design_filename,
                                             project_path=self.repo.workdir)
            task_doc = await Document.load(filename=task_filename,
                                           project_path=self.repo.workdir)
            ...
            for task_filename in task_list:
                ...
                if not old_code_doc:
                    old_code_doc = Document(root_path=str(
                        self.repo.src_relative_path),
                                            filename=task_filename,
                                            content="")
                if not code_plan_and_change_doc:
                    context = CodingContext(filename=task_filename,
                                            design_doc=design_doc,
                                            task_doc=task_doc,
                                            code_doc=old_code_doc)
                else:
        ...
        self.code_todos = [
            WriteCode(i_context=i,
                      repo=self.repo,
                      input_args=self.input_args,
                      context=self.context,
                      llm=self.llm) for i in changed_files.docs.values()
        ]
        ...
        for filename in changed_src_files:
            ...
            self.code_todos.append(
                WriteCode(i_context=coding_doc,
                          repo=self.repo,
                          input_args=self.input_args,
                          context=self.context,
                          llm=self.llm))
    ...
    async def _new_summarize_actions(self):
        ...
        for filename in src_files:
            dependencies = await self.repo.srcs.get_dependency(
                filename=filename)
        ...
        for ctx, filenames in summarizations.items():
            ...
            new_summarize = SummarizeCode(i_context=ctx,
                                          repo=self.repo,
                                          input_args=self.input_args,
                                          context=self.context,
                                          llm=self.llm)
    ...
    async def _new_code_plan_and_change_action(self, cause_by: str):
        ...
        if cause_by != any_to_str(FixBug):
            ...
            fixbug_doc = await Document.load(
                filename=self.input_args.issue_filename)
        ...
        if hasattr(self.input_args, "changed_prd_filenames"):
            code_plan_and_change_ctx = CodePlanAndChangeContext(
                ...
                design_filename=self.input_args.
                changed_system_design_filenames[0],
            ...
            code_plan_and_change_ctx = CodePlanAndChangeContext(
                requirement=options.get("requirement", ""),
                issue=options.get("issue", ""),
                prd_filename=str(self.repo.docs.prd.workdir /
                                 self.repo.docs.prd.all_files[0]),
                design_filename=str(self.repo.docs.system_design.workdir /
                                    self.repo.docs.system_design.all_files[0]),
                task_filename=str(self.repo.docs.task.workdir /
                                  self.repo.docs.task.all_files[0]),
            )
    ...
    async def _is_fixbug(self) -> bool:
        return bool(self.input_args
                    and hasattr(self.input_args, "issue_filename"))
