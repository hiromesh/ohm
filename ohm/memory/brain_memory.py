...
from ohm.config2 import Config as _Config
from ohm.const import DEFAULT_MAX_TOKENS, DEFAULT_TOKEN_SIZE
from ohm.logs import logger
from ohm.provider import ohmLLM
from ohm.provider.base_llm import BaseLLM
from ohm.schema import Message, SimpleMessage
from ohm.utils.redis import Redis
...
class BrainMemory(BaseModel):
    ...
    async def summarize(self, llm, max_words=200, keep_language: bool = False, limit: int = -1, **kwargs):
        if isinstance(llm, ohmLLM):
            return await self._ohm_summarize(max_words=max_words)
    ...
    async def _ohm_summarize(self, max_words=200):
        if not self.history:
            return ""
        ...
        return BrainMemory.to_ohm_history_format(self.history)

    @staticmethod
    def to_ohm_history_format(history) -> str:
    ...
    async def is_related(self, text1, text2, llm):
        if isinstance(llm, ohmLLM):
            return await self._ohm_is_related(text1=text1, text2=text2, llm=llm)
        return await self._openai_is_related(text1=text1, text2=text2, llm=llm)

    @staticmethod
    async def _ohm_is_related(**kwargs):
        return False
    ...
    async def rewrite(self, sentence: str, context: str, llm):
        if isinstance(llm, ohmLLM):
            return await self._ohm_rewrite(sentence=sentence, context=context, llm=llm)
        return await self._openai_rewrite(sentence=sentence, context=context, llm=llm)

    @staticmethod
    async def _ohm_rewrite(sentence: str, **kwargs):
        return sentence
