...
from ohm.const import ohm_REPORTER_DEFAULT_URL
from ohm.logs import create_llm_stream_queue, get_llm_stream_queue
...
class ResourceReporter(BaseModel):
    ...
    callback_url: str = Field(ohm_REPORTER_DEFAULT_URL, description="The URL to which the report should be sent")
    ...
    def _format_data(self, value, name, extra):
        ...
        if role:
            ...
            role_name = os.environ.get("ohm_ROLE")
        data["role"] = role_name
        if extra:
            data["extra"] = extra
        return data
