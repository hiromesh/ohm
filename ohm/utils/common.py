...
from ohm.const import MARKDOWN_TITLE_PREFIX, MESSAGE_ROUTE_TO_ALL
from ohm.logs import logger
from ohm.utils.exceptions import handle_exception
from ohm.utils.json_to_markdown import json_to_markdown
...
class OutputParser:
    ...
    def parse_python_code(text: str) -> str:
        for pattern in (r"(.*?```python.*?\s+)?(?P<code>.*)(```.*?)",
                        r"(.*?```python.*?\s+)?(?P<code>.*)"):
            match = re.search(pattern, text, re.DOTALL)
            if not match:
                continue
    ...
    def extract_content(text, tag="CONTENT"):
        ...
        extracted_content = re.search(rf"\[{tag}\](.*?)\[/{tag}\]", text,
                                      re.DOTALL)
        ...
        if extracted_content:
            ...
            raise ValueError(
                f"Could not find content between [{tag}] and [/{tag}]")
    ...
    def parse_data_with_mapping(cls, data, mapping):
        ...
        for block, content in block_dict.items():
            ...
            if typing == List[str] or typing == List[Tuple[
                    str, str]] or typing == List[List[str]]:
                # 尝试解析list
                try:
                    content = cls.parse_file_list(text=content)
                except Exception:
    ...
    @classmethod
    def extract_struct(
            cls, text: str, data_type: Union[type(list),
                                             type(dict)]) -> Union[list, dict]:
        ...
        if start_index != -1 and end_index != -1:
            ...
            try:
                ...
                raise ValueError(
                    f"The extracted structure is not a {data_type}.")
...
def split_namespace(ns_class_name: str,
                    delimiter: str = ":",
...
def general_after_log(
        i: "loguru.Logger",
        sec_format: str = "%0.3f") -> Callable[["RetryCallState"], None]:
    """
...
def write_json_file(json_file: str,
                    data: Any,
                    encoding: str = "utf-8",
                    indent: int = 4,
...
def read_jsonl_file(jsonl_file: str, encoding="utf-8") -> list[dict]:
    if not Path(jsonl_file).exists():
        raise FileNotFoundError(
            f"json_file: {jsonl_file} not exist, return []")
    datas = []
...
def import_class_inst(class_name: str, module_name: str, *args,
                      **kwargs) -> object:
...
async def async_read(filename: str | Path, encoding="utf-8") -> str:
    """Read file asynchronously."""
    if not filename or not Path(filename).exists():
        return ""
    try:
        async with aiofiles.open(str(filename), mode="r",
...
async def async_write(filename: str | Path, data: str, encoding="utf-8"):
    """Write file asynchronously."""
    pathname = Path(filename)
    pathname.parent.mkdir(parents=True, exist_ok=True)
    async with aiofiles.open(str(pathname), mode="w",
...
def parse_json_code_block(markdown_text: str) -> List[str]:
    json_blocks = (re.findall(r"```json(.*?)```", markdown_text, re.DOTALL)
                   if "```json" in markdown_text else [markdown_text])
...
def load_mc_skills_code(skill_names: list[str] = None,
                        skills_dir: Path = None) -> list[str]:
    """load minecraft skill from js files"""
    if not skills_dir:
        skills_dir = Path(__file__).parent.absolute()
    if skill_names is None:
        skill_names = [
            skill[:-3] for skill in os.listdir(f"{skills_dir}")
...
def encode_image(image_path_or_pil: Union[Path, Image, str],
...
async def get_mime_type(filename: str | Path, force_read: bool = False) -> str:
    ...
    from ohm.tools.libs.shell import shell_execute  # avoid circular import
    ...
    try:
        stdout, stderr, _ = await shell_execute(
            f"file --mime-type '{str(filename)}'")
        if stderr:
...
def get_markdown_codeblock_type(filename: str = None,
...
async def save_json_to_markdown(content: str, output_filename: str | Path):
    ...
    try:
        ...
        return
    await async_write(filename=output_filename, data=json_to_markdown(m))
...
def new_transaction_id(postfix_len=8) -> str:
    ...
    return datetime.now().strftime(
        "%Y%m%d%H%M%ST") + uuid.uuid4().hex[0:postfix_len]
...
def log_time(method):
    ...
    def before_call():
        ...
        logger.info(f"[{method.__name__}] started at: "
    ...
    return timeit_wrapper_async if iscoroutinefunction(
