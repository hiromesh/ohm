...
from ohm.utils.tree import tree
...
async def repo_to_markdown(repo_path: str | Path, output: str | Path = None) -> str:
    ...
    if output:
        ...
        await async_write(filename=str(output_file), data=markdown, encoding="utf-8")
...
async def _write_file(filename: Path, repo_path: Path) -> str:
    ...
    try:
        ...
        content = await async_read(filename, encoding="utf-8")
