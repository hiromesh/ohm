...
from ohm.actions import WriteCode
from ohm.actions.action import Action
from ohm.logs import logger
from ohm.schema import CodingContext, Document
from ohm.tools.tool_registry import register_tool
from ohm.utils.common import CodeParser, async_read, async_write
...
class ValidateAndRewriteCode(Action):
    ...
    async def run(
        ...
        code, design_doc, task_doc = await asyncio.gather(
            async_read(code_path), self._try_aread(system_design_input), self._try_aread(project_schedule_input)
        )
        ...
        await async_write(filename=code_path, data=code)
