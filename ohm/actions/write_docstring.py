...
Usage:
    python3 -m ohm.actions.write_docstring <filename> [--overwrite] [--style=<docstring_style>]
...
Example:
    python3 -m ohm.actions.write_docstring ./ohm/software_company.py --overwrite False --style=numpy
...
from ohm.utils.common import OutputParser, async_read, async_write
from ohm.utils.pycst import merge_docstring
...
class WriteDocstring(Action):
    ...
    async def write_docstring(
        ...
        data = await async_read(str(filename))
        code = await WriteDocstring().run(data, style=style)
        if overwrite:
            await async_write(filename, code)
        return code
